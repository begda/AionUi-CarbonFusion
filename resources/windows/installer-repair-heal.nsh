...
!macro AIONUI_READ_LAST_INNER_FAILURE
  ...
  StrCpy $AionUiInnerFailureSummary "No specific locking process was identified. Close CarbonFusion, terminals, editors, and file managers opened in the install folder."
  nsExec::ExecToStack `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
    ...
    $$summary = 'No specific locking process was identified. Close CarbonFusion, terminals, editors, and file managers opened in the install folder.'; \
    $$code = ''; \
