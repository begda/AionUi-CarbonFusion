...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'carbonfusion-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'carbonfusion-installer-report.json'
...
function Read-CarbonFusionAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'CarbonFusion\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'CarbonFusion\analytics.json')
  }
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('carbonfusion-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('carbonfusion-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('CarbonFusion installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
  ...
  $lines.Add('---------------------------')
  $lines.Add('To CarbonFusion Team')
  $lines.Add('---------------------------')
...
try {
  ...
  $userId = Read-CarbonFusionAnalyticsId
  $eventPayload = @{
    message = ('installer-failure ' + $Code)
    level = 'error'
    platform = 'other'
    release = $Release
    logger = 'carbonfusion.installer'
  ...
  Show-ReportMessage ('CarbonFusion installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Tip: If you can get in touch with the CarbonFusion team, press [ Ctrl + C ] in this dialog to copy details, then send them to us via social media, email, or a GitHub issue:' + [Environment]::NewLine + 'https://github.com/iOfficeAI/CarbonFusion/issues' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('CarbonFusion installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
