...
for ($attempt = 1; $attempt -le 5; $attempt++) {
  ...
  if ($attempt -lt 5) {
    ...
    $summaryBase64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($summary))
    Write-Output "verify-bundled-aioncore kind=resource-failure detailBase64=$summaryBase64"
  }
}
