...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    code: 'E1001',
    message: 'CarbonFusion could not repair the installed uninstaller.',
    action: 'Close CarbonFusion, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    code: 'E1002',
    message: 'The previous CarbonFusion uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    code: 'E1003',
    message: 'CarbonFusion could not remove or replace the previous installation directory.',
    action: 'Close CarbonFusion and any program using the install folder, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1010',
    message: 'CarbonFusion could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=CarbonFusion.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'AIONUI_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'CarbonFusion cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    code: 'E1030',
    message: 'CarbonFusion installed, but the bundled AionCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
  ...
  },
  {
    ...
    code: 'E1031',
    message: 'CarbonFusion installation is incomplete because a required application file is missing.',
    action: 'Reinstall CarbonFusion or download a newer installer.',
  ...
  },
  {
    ...
    action: 'Download the CarbonFusion installer that matches this Windows architecture, then run it again.',
  ...
  },
  {
    ...
    code: 'E1050',
    message: 'Another CarbonFusion installer appears to still be active.',
  ...
  },
  {
    ...
    code: 'E1060',
    message: 'CarbonFusion found an invalid previous-install registry state.',
    action: 'Uninstall the old CarbonFusion from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1070',
    message: 'CarbonFusion could not write the active-installer marker.',
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*CarbonFusion installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "CarbonFusion installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "CarbonFusion Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\CarbonFusion-messagebox-smoke"
  StrCpy $CarbonFusionSessionId "smokembox-${nsisQuote(scenario.code)}"
  StrCpy $CarbonFusionIsUpdated "1"
  StrCpy $CarbonFusionSessionLogPath "${nsisQuote(logPath)}"
  BringToFront
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (
          ...
          !status.copyText.includes(`CarbonFusion installer failure ${code}`)
        ) {
